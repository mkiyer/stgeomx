...
x <- cpm(st_geomx_bgcorrect(ds, method="qq", bw.adjust=2))
p <- st_geomx_plot_bgcorrect(ds, x)
p
f <- file.path(plot_dir, "qc_bgcorrect_qq.pdf")
ggsave(f, plot=p, device="pdf", width=8, height=12)

p <- st_geomx_plot_bgcorrect_density(ds, x)
p
f <- file.path(plot_dir, "qc_bgcorrect_q_density.pdf")
ggsave(f, plot=p, device="pdf", width=8, height=12)


x <- cpm(st_geomx_bgcorrect(ds, method="bgsub"))
p <- st_geomx_plot_bgcorrect(ds, x)
p
f <- file.path(plot_dir, "qc_bgcorrect_bgsub.pdf")
ggsave(f, plot=p, device="pdf", width=8, height=12)

p <- st_geomx_plot_bgcorrect_density(ds, x)
p
f <- file.path(plot_dir, "qc_bgcorrect_bgsub_density.pdf")
ggsave(f, plot=p, device="pdf", width=8, height=12)
...
reskdefdr <- bgcorrect_kdefdr2(xs, bg, bw.adjust=bw.adjust, fdr.min=0.05)$y
resnorm <- pmax(1, bgcorrect_norm(xs, bg))
resbgsub <- pmax(1, xs - geomean(xs[bg]))
resqq <- bgcorrect_qq(xs, bg, bw.adjust=bw.adjust)$y
...
tbl <- bind_rows(bind_cols(value = xs, bgcorrect="before", bg=bg),
                 ...
                 bind_cols(value = resbgsub, bgcorrect="norm2", bg=bg),
                 bind_cols(value = reskdefdr, bgcorrect="bgsub", bg=bg),
...
tbl <- bind_cols(xs=xs, ynorm=resnorm, yfdr=reskdefdr, yqq=resqq, bg=bg)
ggplot(tbl) + 
  ...
  geom_point(data=filter(tbl, bg), aes(xs, yfdr), size=3, alpha=0.5, color="red") +
  geom_line(data=filter(tbl, !bg), aes(xs, yfdr), color="red") +
...
res <- bgcorrect_qq(xs, bg, bw.adjust = 2)
tbl <- bind_cols(x=log2(xs), noise=log2(res$noise), y=log2(res$y))
...
## BG correction version 3 (kdefdr3)
...
# determine expressed/not expressed probes
bg.cutoff <- quantile(x[bg], bg.quantile)
table(bg, x < bg.cutoff)
noise <- bg | (x < bg.cutoff)

x <- log2(x)
xmin <- min(x)
xmax <- max(x)

d <- density(x[!noise], adjust=bw.adjust, from=xmin, to=xmax, n=n)
dbg <- density(x[noise], adjust=bw.adjust, from=xmin, to=xmax, n=n)

dcdf <- cumsum(d$y) / sum(d$y)
dbgcdf <- cumsum(dbg$y) / sum(dbg$y)
#pnoise <- dbgcdf / (dcdf + dbgcdf)
pnoise <- d$y / (d$y + dbg$y + eps)

# ensure isotonic behavior
ir <- isoreg(d$x, pnoise)
...
ggplot(tbl) +
  geom_line(aes(x,d)) +
  geom_line(aes(x,dbg), color="red")

ggplot(tbl) +
  geom_line(aes(x,d)) +
  geom_line(aes(x,dbg), color="red") +
  geom_line(aes(x,pnoise), color="blue") + 
  geom_line(aes(x,yf), color="magenta")
...
d <- density(x[!noise], adjust=bw.adjust, n=n)
dcdf <- cumsum(d$y) / sum(d$y)
