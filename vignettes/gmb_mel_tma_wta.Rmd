...
## BG correction using normal approximation

```{r bg correction normal approximation}


st_geomx_bgcorrect_norm <- function(x, bg) {

  apply_bgcorrect <- function(x) {
...
ggplot(tbl) +
  ...
  geom_line(aes(dx, 2* pbg), color="blue")
...
ggplot(tbl) + 
  geom_point(data=filter(tbl, bg), aes(x1, x1-y1), color="red") +
  geom_line(data=filter(tbl, !bg), aes(x1, x1-y1), color="black") +
#  geom_line(aes(x1, x1-y1), color="red") +
  geom_hline(yintercept = mean((x1-y1)[bg]), color="red", linetype="dashed") +
...
estimate_noise_kde <- function(x, bg, bw, n) {
  xmin <- min(x)
  xmax <- max(x)
  ...
  deps <- min(d$y[d$y > 0])
  dbgeps <- min(dbg$y[dbg$y > 0])
  lr <- (d$y + deps) / (dbg$y + dbgeps)
  pbg <- 1 / (1 + lr)
  pbgcdf <- cumsum(pbg)
  noise <- xmin + (xmax - xmin) * pbgcdf / n
  y <- approx(d$x, noise, xout=x)$y
  return(list(nx=d$x, ny=noise, y=y))
}
...
d <- density(x1[!bg], adjust=bw.adjust, bw=bw, from=xmin, to=xmax, n=n)
dbg <- density(x1[bg], adjust=bw.adjust, bw=bw, from=xmin, to=xmax, n=n)
deps <- min(d$y[d$y > 0])
dbgeps <- min(dbg$y[dbg$y > 0])
lr <- (d$y + deps) / (dbg$y + dbgeps)
pbg <- 2 / (1 + lr)
...
tbl <- bind_rows(bind_cols(value = x1, bgcorrect="before", bg=bg),
                 bind_cols(value = y1, bgcorrect="noise", bg=bg), 
                 bind_cols(value = x1-y1, bgcorrect="after", bg=bg))
...
plot_bgsub <- function(s, m, x, cpm_quantile = 0.75) {
...
#    scale_x_log10() +
    ...
    labs(color = "BG", shape = "Keep AOI", x="CPM", y="AOI",
  ...
  p2 <- ggplot(y, aes(x=qcpm_TRUE, y=qcpm_FALSE, color=keep_aoi)) +
...
# plot bg
x <- cpm(ds$counts)
p1 <- plot_bgsub(ds$samples, ds$meta, x, cpm_quantile)
p1
...
bg <- ds$meta$bg
x <- log2(cpm(ds$counts))
x <- st_geomx_bgcorrect(x, bg)
p2 <- plot_bgsub(ds$samples, ds$meta, x, cpm_quantile)
p2
...
# bgsub
x <- cpm(background_subtract(ds$counts, ds$samples$bg_geomean))
p3 <- plot_bgsub(ds$samples, ds$meta, x, cpm_quantile)
p3


p <- p1 / p3
p
f <- file.path(plot_dir, "qc_aoi_qc_bgsub.pdf")
...
# normalize
#bgnorm_cpm <- bgnorm(fds)
#bgnorm_qn_cpm <- normalize_quantile(bgnorm_cpm)
bgsub_qn_cpm <- st_geomx_normalize(fds, bgsub=TRUE, method="qn")
bgsub_rle_cpm <- st_geomx_normalize(fds, bgsub=TRUE, method="rle")

#rownames(bgnorm_cpm) <- fds$meta$gene
#rownames(bgnorm_qn_cpm) <- fds$meta$gene
rownames(bgsub_qn_cpm) <- fds$meta$gene
rownames(bgsub_rle_cpm) <- fds$meta$gene

# try a different method
# y <- background_subtract(fds$counts, fds$samples$bg_geomean)
# y <- DGEList(counts = y, samples=fds$samples, genes=fds$meta)
# y <- calcNormFactors(y, method="TMM")
# v <- voom(y, plot=TRUE)

# choose normalized data to work with
normcounts <- bgsub_rle_cpm
#normcounts <- bgsub_qn_cpm
#normcounts <- bgnorm_qn_cpm
#normcounts <- bgnorm_cpm
...
s <- select(ds$samples, aoi, slide, keep, acral, sample_type, os_quartile, vital_status)
x <- bind_cols(ds$meta, ds$counts)
x <- x %>%
...
p1 <- p + geom_point(aes(color=frac_expr), alpha = 0.8, size=2) +
  scale_color_viridis_c()
p1
...
p <- fviz_pca_ind(res.pca, select.ind=list(contrib=50), repel=TRUE, max.overlaps=Inf)
p
ggsave(file.path(plot_dir, "pca_ind.pdf"), p)

p <- fviz_contrib(res.pca, choice = "ind", axes = 1, top=20)
p
...
p <- plot_gene(s, y, "AFAP1L2", os_quartile, os_quartile)
