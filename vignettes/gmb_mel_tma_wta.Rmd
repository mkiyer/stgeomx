...
x <- cpm(st_geomx_bgcorrect(ds, method="wfpr", bw.adjust=2, w=1))
p <- st_geomx_plot_bgcorrect(ds, x)
p
f <- file.path(plot_dir, "qc_bgcorrect_wfpr.pdf")
ggsave(f, plot=p, device="pdf", width=8, height=12)

p <- st_geomx_plot_bgcorrect_density(ds, x)
p
f <- file.path(plot_dir, "qc_bgcorrect_wfpr_density.pdf")
ggsave(f, plot=p, device="pdf", width=8, height=12)
...
# bg correct
x <- st_geomx_bgcorrect(fds, method="wfpr", bw.adjust=2, w=1)
...
res <- bgcorrect_wfpr(x1, bg, bw.adjust=bw.adjust, w=w)
...
tbl <- bind_rows(bind_cols(value = x1, bgcorrect="before", bg=bg),
                 bind_cols(value = res$y, bgcorrect="kde", bg=bg),
...
ggplot(tbl) +
  geom_line(aes(dx, dy)) +
  geom_line(aes(dx, dwfpr), color="blue")

ggplot(tbl) +
  geom_line(aes(dx, min(log2(x1)) + (max(log2(x1)) - min(log2(x1))) * cumsum(dwfpr)/10000))
...
ggplot(tbl) +
  ...
  geom_line(aes(dx, -log10(dwfpr)), color="blue")
