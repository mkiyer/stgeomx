...
x <- cpm(st_geomx_bgcorrect(ds, method="kdefdr", bw.adjust=3, fdr.min=0.1))
p <- st_geomx_plot_bgcorrect(ds, x)
p
p <- st_geomx_plot_bgcorrect_density(ds, x)
p
...
#x <- st_geomx_bgcorrect(fds, method="geonorm")
#x <- st_geomx_bgcorrect(fds, method="norm")
x <- st_geomx_bgcorrect(fds, method="kdefdr", bw.adjust=3, fdr.min=0.1)
...
score <- (abs(cor(pca_tbl$PC1, pca_tbl$bg_cpm)) * pct_var[1] +
            abs(cor(pca_tbl$PC2, pca_tbl$bg_cpm)) * pct_var[2] +
            abs(cor(pca_tbl$PC3, pca_tbl$bg_cpm)) * pct_var[3] +
            abs(cor(pca_tbl$PC4, pca_tbl$bg_cpm)) * pct_var[4])
score

ggplot(pca_tbl, aes(bg_cpm, PC1)) + 
  geom_point() + 
  geom_smooth(method='lm', formula= y~x) +
  labs(y=pct_var_lbl[1], title=cor.test(pca_tbl$PC1, pca_tbl$bg_cpm)$estimate)
ggplot(pca_tbl, aes(bg_cpm, PC2)) + 
  geom_point() + 
  geom_smooth(method='lm', formula= y~x) +
  labs(y=pct_var_lbl[2], title=cor.test(pca_tbl$PC2, pca_tbl$bg_cpm)$estimate)
ggplot(pca_tbl, aes(bg_cpm, PC3)) + 
  geom_point() + 
  geom_smooth(method='lm', formula= y~x) +
  labs(y=pct_var_lbl[3], title=cor.test(pca_tbl$PC3, pca_tbl$bg_cpm)$estimate)

p <- ggplot(pca_tbl, aes(x=PC1, y=PC2)) +
  theme_minimal() +
  labs(x=pct_var_lbl[1], y=pct_var_lbl[2], title=100*score) +
  geom_point(aes(color=bg_cpm), alpha = 0.8, size=2) +
...
## DE analysis across different patients
...
gsea %>% filter(analysis == "os_q0_vs_q3", pathway=="HALLMARK_OXIDATIVE_PHOSPHORYLATION") %>% pull(leadingEdge)


p <- plot_gsea_barplot(filter(gsea, analysis == "os_q0_vs_q3"))
p
ggsave(file.path(plot_dir, "gsea_hallmark_os_q0_vs_q3.pdf"), p, height=8)

p <- plot_gsea_barplot(filter(gsea, analysis == "acral"))
p
...
# gsea gobp
prefix <- "gsea_gobp"
gsea <- fgsea(pathways=gs_gobp, stats=ranks, minSize=10, eps=0, nPermSimple=10000)
gsea <- as_tibble(gsea)
...
x <- gsea %>% filter(pathway %in% top50pathways)
x$pathway <- gsub("GOBP_", "", x$pathway)
x$pathway <- substr(x$pathway, 1, 50)

p <- plot_gsea_barplot(x) +
  labs(title = "GOBP gene sets",
       subtitle = "Published RNA-Seq cohorts")
p


for (i in 1:nrow(gsea)) {
  x <- gsea[i,]
  p <- plot_gsea_enrichment(x, ranks, cptac_gs)
  f <- file.path(plot_dir, paste0("st_pdac_", x$pathway, ".pdf"))
  ggsave(f, plot=p, device="pdf", width=5, height=3)
}


table(de$analysis)
analysis = "os_q0_vs_q3"
ranks <- get_de_ranks(de, analysis)

gsea <- fgsea(pathways=gs_hallmark, stats=ranks, minSize=10, eps=0, nPermSimple=10000)
gsea <- fgsea(pathways=gs_hpa, stats=ranks, minSize=10, eps=0, nPermSimple=10000)
gsea <- fgsea(pathways=gs_msigdb, stats=ranks, minSize=10, eps=0, nPermSimple=10000)
...
```{r exploratory bgcorrect work}

bad_aoi <- ds$samples %>% arrange(bg_auc) %>% head(1) %>% pull(aoi)
good_aoi <- ds$samples %>% arrange(desc(bg_auc)) %>% head(1) %>% pull(aoi)
bg <- ds$meta$bg
# between 0 and 0.5
theta <- 0.25

x <- unlist(ds$counts[, bad_aoi])
x <- log2(x)
xbg <- x[bg]

xnew <- bgcorrect(x, xbg, theta=0.25, bw.adjust=3, n=10000, eps=1e-10)
plot(x, xnew)

xmin <- min(x)
xmax <- max(x)
dbg <- density(xbg, adjust=3, from=xmin, to=xmax, n=10000)
ybg <- cumsum(dbg$y) / sum(dbg$y)
qbg <- approx(dbg$x, ybg, xout=x, rule=2)$y


idx <- which(xbg >= quantile(xbg, theta) & xbg <= quantile(xbg, 1-theta))
xbgsubset <- xbg[idx]
ybgsubset <- (rank(xbg)/length(xbg))[idx]
out <- lm(xbgsubset ~ ybgsubset)
...
x - ybg
plot(x, ybg)
plot(xbgsubset, ybgsubset)

bad_aoi <- ds$samples %>% arrange(bg_auc) %>% head(1) %>% pull(aoi)
good_aoi <- ds$samples %>% arrange(desc(bg_auc)) %>% head(1) %>% pull(aoi)
bg <- ds$meta$bg

x <- unlist(ds$counts[, good_aoi])
x <- log2(x)
xbg <- x[bg]
#xmin <- min(x)
#xmax <- max(x)
...
#d <- density(x, adjust=3, from=xmin, to=xmax, n=10000)
dbg <- density(xbg, adjust=3, from=xmin, to=xmax, n=10000)
eps <- 1e-10
ybg <- cumsum(dbg$y + eps) / sum(dbg$y + eps)
qbg <- approx(dbg$x, ybg, xout=x, rule=2)$y

approx(ybg, dbg$x, xout = qbg)
...
plot(approx(ybg, dbg$x, xout = qbg), pch=19, cex=0.5)


plot(dbg$x, ybg)
plot(x, y)
xnew <- x - approx(ybg, dbg$x, xout=qbg, rule=2)$y
plot(x, quantile(xbg, probs=qbg, names=FALSE), pch=19, cex=0.5)
xnew <- x - quantile(xbg, probs=qbg, names=FALSE)
plot(x, xnew)


outx <- density(x, adjust=3)
data <- bind_cols(x=outx$x, y=outx$y)
databg <- bind_cols(x=outbg$x, y=outbg$y)
p <- ggplot() + 
  geom_line(data=databg, aes(x, y)) +
...
reskdefdr <- bgcorrect_kdefdr2(x1, bg, bw.adjust=3, fdr.min=0.1)$y
resnorm <- pmax(1, bgcorrect_norm(x1, bg))
resgeonorm <- pmax(1, bgcorrect_geonorm(x1, bg))
...
tbl <- bind_rows(bind_cols(value = x1, bgcorrect="before", bg=bg),
                 ...
                 bind_cols(value = resgeonorm, bgcorrect="geonorm", bg=bg),
                 bind_cols(value = resbgsub, bgcorrect="bgsub", bg=bg),
                 bind_cols(value = reskdefdr, bgcorrect="kdefdr", bg=bg))
ggplot(tbl, aes(x=value, y=factor(bgcorrect), fill=factor(bg))) +
  stat_density_ridges(alpha=0.5) +
  scale_fill_manual(values = pals::cols25()) + 
  scale_x_log10()

tbl <- bind_cols(x1=x1, ynorm=resnorm, ygeonorm=resgeonorm, yfdr=reskdefdr, bg=bg)
ggplot(tbl) + 
  geom_point(data=filter(tbl, bg), aes(x1, ynorm), size=3, alpha=0.5, color="blue") +
  geom_line(data=filter(tbl, !bg), aes(x1, ynorm), color="blue") +
  geom_point(data=filter(tbl, bg), aes(x1, ygeonorm), size=3, alpha=0.5, color="red") +
  geom_line(data=filter(tbl, !bg), aes(x1, ygeonorm), color="red") +
  geom_point(data=filter(tbl, bg), aes(x1, yfdr), size=3, alpha=0.5, color="green") +
  geom_line(data=filter(tbl, !bg), aes(x1, yfdr), color="green") +
...
descdist(x[bg], discrete = FALSE, boot=1000)
fitbg <- MASS::fitdistr(x[bg], "logistic")
fitbg$estimate["location"]
fitbg$estimate["scale"]
xbg <- rlogis(length(x[!bg]), location=fitbg$estimate["location"], scale=fitbg$estimate["scale"])
...
tbl <- bind_cols(x=x1, y=y, noise=2^noise, bg=bg)
ggplot(tbl) + 
  geom_point(data=filter(tbl, bg), aes(x, y), size=3, alpha=0.5, color="red") +
  geom_point(data=filter(tbl, !bg), aes(x, y), size=1, alpha=0.3) +
  geom_line(data=filter(tbl, !bg), aes(x, y), alpha=0.5) +
  geom_line(aes(x, noise), color="blue") +
...
```{r explore silhouette width approach}

clustering <- c(rep(1, i), rep(2, length(xvals) - i))


ai <- NULL
for (j in 1:i) {
  ai[j] <- d1sum[j] / (c1n - xlens[j])
}
for (j in (i+1):length(xvals)) {
  ai[j] <- d2sum[j] / (c2n - xlens[j])
}
...
tbl <- bind_cols(x=xvals, ai=ai, bi=bi, si=si, clustering=factor(clustering))
ggplot(tbl) + 
  geom_point(aes(x, ai, color=clustering))
ggplot(tbl) + 
  geom_point(aes(x, bi, color=clustering))
ggplot(tbl) + 
  geom_point(aes(x, si, color=clustering))



i <- sum(bg)
clust <- c(rep(1, i), rep(2, length(xs) - i))
...
cluster::silhouette(list(x=c(1,2,3,4,5,6), clustering=c(1,1,1,2,2,2)),
                    dist=dist(c(1,2,3,4,5,6)))
...
## Explore likelihood ratio transform

```{r likelihood ratio explore}

# 
# x <- ds$counts[!ds$meta$bg, ]
# xbg <- ds$counts[ds$meta$bg, ]
# 
# x1 <- log2(unlist(x[,1]))
# xbg1 <- log2(unlist(xbg[,1]))
# xmin <- min(c(x1, xbg1))
# xmax <- max(c(x1, xbg1))
# hist(x1, freq=FALSE, breaks=100, ylim=c(0, 1))
# lines(density(x1, bw=0.2))
# hist(xbg1, freq=FALSE, breaks=10, ylim=c(0, 2))
# lines(density(xbg1, bw=0.2))
# out <- density(x1, bw=0.2, from=xmin, to=xmax)
# outbg <- density(xbg1, bw=0.2, from=xmin, to=xmax)
# p <- 15
# y <- approx(out$x, out$y, xout = p)$y
# ybg <- approx(outbg$x, outbg$y, xout = p)$y
# y / ybg
# 
```
...
oldbgquants <- function(x, xbg, bw.adjust=3, n=10000, eps=1e-10) {
  # quantiles of background signal corresponding to signal
  xmin <- min(x)
  xmax <- max(x)
  d <- density(xbg, adjust=bw.adjust, from=xmin, to=xmax, n=n)
  bgcdf <- d$y + eps
  bgcdf <- cumsum(d$y) / sum(d$y)
  q <- approx(d$x, bgcdf, xout=x)$y
  #q <- approx(d$x, bgcdf, xout=x, rule=2)$y
  return(q)
}


oldbglm <- function(xbg, theta) {
  # linear model of background in quantile window [theta, 1-theta]
  idx <- which(xbg >= quantile(xbg, theta) & xbg <= quantile(xbg, 1-theta))
  xbgsubset <- xbg[idx]
  ybgsubset <- (rank(xbg, ties.method="random")/length(xbg))[idx]
  out <- lm(xbgsubset ~ ybgsubset)
  b <- out$coefficients[1]
  m <- out$coefficients[2]
...
old_st_geomx_bgcorrect <- function(x, bg, theta=0.25, bw.adjust=3, n=10000, eps=1e-10, min.count=1) {

  apply_bgcorrect <- function(x, bg) {
    x <- log2(x)
    qbg <- bgquants(x, x[bg], bw.adjust, n, eps)
    noise_func <- bglm(x[bg], theta)
    ybg <- noise_func(qbg)
    return(2^(x) - 2^(ybg))
    #return(x - ybg)
  }  
  
  x <- apply(x, MARGIN=2, FUN=apply_bgcorrect)
...
d <- density(x1, bw=bw, from=x1min, to=x1max, n=n)
dbg <- density(x1bg, bw=bw, from=x1min, to=x1max, n=n)
lr <- d$y / dbg$y
pbg <- 1 / (1 + lr)
pbgcdf <- cumsum(pbg)
noise <- x1min + (x1max - x1min) * pbgcdf / n

y1 <- approx(d$x, noise, xout=x1)$y

plot(x1, x1 - y1, cex=0.1, pch=19)

tbl <- bind_cols(dx=d$x, dy=d$y, dbgx=dbg$x, dbgy=dbg$y, lr=lr, 
                 pbg=pbg, pbgcdf=pbgcdf, noise=noise)
ggplot(tbl) + 
  geom_line(aes(dx, noise)) +
  geom_hline(yintercept = median(x1bg), color="red", linetype="dashed")

tbl <- bind_cols(x1=x1, y1=y1)
ggplot(tbl) + 
  geom_line(aes(x1,y1)) +
  geom_line(aes(x1, x1-y1), color="red") +
  geom_hline(yintercept = median(x1bg), color="red", linetype="dashed") +
  geom_hline(yintercept = mean(x1bg), color="blue", linetype="dashed")
...
y <- approx(d$x, d$y, xout=x1)$y
ybg <- approx(dbg$x, dbg$y, xout=x1)$y

lr <- y / ybg
pbg <- 1 / (1 + lr)
cumsum(pbg)

plot(x1, cumsum(pbg), xlim=c(0,1000))

tbl <- bind_cols(dx=d$x, dy=d$y, dbgx=dbg$x, dbgy=dbg$y)
ggplot(tbl) + 
  geom_line(aes(dbgx, dbgy)) +
  xlim(1, 1000)
...
ggplot(tbl) +
  geom_line(aes(dx, dy)) +
  geom_line(aes(dbgx, dbgy), color="red")



dcdf <- cumsum(d$y + eps) / sum(d$y + eps)
dbgcdf <- cumsum(dbg$y + eps) / sum(dbg$y + eps)

y <- approx(d$x, dcdf, xout=x1)$y
ybg <- approx(d$x, dbgcdf, xout=x1)$y

y <- approx(d$x, dcdf, xout=10)$y
ybg <- approx(d$x, dbgcdf, xout=10)$y
...
tbl <- bind_cols(dx = d$x, dy = dcdf, dbgx=dbg$x, dbgy=dbgcdf)
ggplot(tbl) +
  geom_line(aes(dx, dy)) +
  geom_line(aes(dbgx, dbgy), color="red")
...
bgdensity <- density(x1bg, adjust=3, from=xmin, to=xmax, n=100000)
bgcdf <- cumsum(bgdensity$y + eps) / sum(bgdensity$y + eps)
bgq <- approx(bgdensity$x, bgcdf, xout=x1, rule=2)$y

plot(density(bgq))
plot(ecdf(bgdensity$y))

y <- approx(bgcdf, bgdensity$x, xout=q)$y
plot(bgcdf, bgdensity$x)
plot(x1, y)
plot(d$x, bgcdf, cex=0.5, pch=19)

qbg <- bgquants(x1, x1[bg])
noise_func <- bglm(x1[bg], theta=0.4)
...
xnew <- (2^x1 - 2^ybg)
xnew <- pmax(xnew, 1)
x <- st_geomx_bgcorrect(ds, theta=0.1, bw.adjust=3, n=10000, eps=1e-10)
