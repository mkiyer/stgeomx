...
x <- cpm(st_geomx_bgcorrect(ds, method="kdefdr", bw.adjust=1, fdr.min=0.05))
p <- st_geomx_plot_bgcorrect(ds, x)
p
p <- st_geomx_plot_bgcorrect_density(ds, x)
p
...
#x <- st_geomx_bgcorrect(fds, method="kdefdr", bw.adjust=2, fdr.min=0.05)
...
ks.test(x=sample(xs, size=sum(bg), replace=FALSE), y=xs[bg], alternative="less")

reskdefdr <- bgcorrect_kdefdr2(xs, bg, bw.adjust=1, fdr.min=0.05)$y
resnorm <- pmax(1, bgcorrect_norm(xs, bg))
resnorm2 <- pmax(1, bgcorrect_norm2(xs, bg))
...
tbl <- bind_rows(bind_cols(value = xs, bgcorrect="before", bg=bg),
                 ...
                 bind_cols(value = resnorm2, bgcorrect="norm2", bg=bg),
...
tbl <- bind_cols(xs=xs, ynorm=resnorm, ynorm2=resnorm2, yfdr=reskdefdr, bg=bg)
ggplot(tbl) + 
  ...
  geom_point(data=filter(tbl, bg), aes(xs, ynorm2), size=3, alpha=0.5, color="red") +
  geom_line(data=filter(tbl, !bg), aes(xs, ynorm2), color="red") +
  geom_point(data=filter(tbl, bg), aes(xs, yfdr), size=3, alpha=0.5, color="green") +
  geom_line(data=filter(tbl, !bg), aes(xs, yfdr), color="green") +
