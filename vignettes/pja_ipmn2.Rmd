...
geomx_annot_xlsx <- file.path(geomx_dir, "annotations_2022-12-02.xlsx")
...
table(ds$samples$class)

# colnames(ds$samples)
# ggplot(ds$samples, aes(x=bg_geomean, y=count_q3, color=segment)) +
...
s <- select(fds$samples, aoi, slide, keep, path, class)
...
# by path
p <- x %>%
  ggplot(aes(x=count, y=factor(path), fill=factor(path))) +
...
# by class
p <- x %>%
  ggplot(aes(x=count, y=factor(path), fill=factor(class))) +
  stat_density_ridges(alpha=0.7, scale=3, rel_min_height=0.001, quantile_lines=TRUE) + 
  scale_color_manual(values = pals::cols25()) + 
  scale_x_log10() +
  theme_ridges() + 
...
# segment / path
p1 <- p + geom_point(aes(color=path), alpha = 0.8, size=2) +
  scale_color_manual(values = pals::cols25())
p1
...
# segment / path
p1 <- p + geom_point(aes(color=path, shape=class), alpha = 0.8, size=2) +
  scale_color_manual(values = pals::cols25())
p1
...
## DE analysis: pathology

```{r between de analysis pathology}

s <- fds$samples %>% filter(segment == "panck")
s$path <- factor(tolower(s$path), levels=c("panck_nmld", "panck_lgd", "panck_hgd", "panck_inv"))
