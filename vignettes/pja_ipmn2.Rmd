...
geomx_annot_xlsx <- file.path(geomx_dir, "pja_ipmn2_annotations_2023-02-06.xlsx")
...
p <- ggplot(ds$samples) + 
  geom_point(aes(num_counts, bg_auc, color=bg_ks_dist)) + 
  geom_vline(xintercept = geomx_min_counts, color="red", linetype="dashed") +
  geom_hline(yintercept = geomx_min_auc, color="red", linetype="dashed") +
  scale_x_log10() +
...
p
ggsave(file.path(plot_dir, "qc_aoi_filter_subset.pdf"), p)


colnames(ds$samples)
p <- ggplot(filter(ds$samples, subset=="epithelial")) + 
  geom_point(aes(AOINucleiCount, AOISurfaceArea, color=bg_ks_dist)) + 
  geom_vline(xintercept = geomx_min_counts, color="red", linetype="dashed") +
  geom_hline(yintercept = geomx_min_auc, color="red", linetype="dashed") +
  scale_x_log10() +
  scale_color_viridis_c() +
...
for (i in s) {
  ...
  res <- bgcorrect_kdenpv(x, bg, bw.adjust=2, bg.quant=0.0)
