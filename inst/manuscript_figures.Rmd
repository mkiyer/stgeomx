...
# load package
install.packages(working_dir, repos = NULL, type = "source")
...
read_msigdb_json_file <- function(filename) {
  ...
  y <- tibble(gs_name = names(x), gs = x) %>% 
    unnest_wider(gs) %>%
    unnest_longer(geneSymbols) %>%
    dplyr::select(gs_cat = collection,
           gs_name = gs_name,
           gene_symbol = geneSymbols)
}
...
f <- system.file("extdata", "msigdb.v2024.1.Hs.json", package = "stgeomx")
msigdb_gene_sets <- read_msigdb_json_file(f)
...
cp_cat <- c("C2:CP:BIOCARTA", "C2:CP:KEGG_MEDICUS", "C2:CP:PID", "C2:CP:REACTOME",
            "C2:CP:WIKIPATHWAYS")
gs_cp <- dplyr::filter(msigdb_gene_sets, gs_cat %in% cp_cat)
gs_cp <- split(x = gs_cp$gene_symbol, f = gs_cp$gs_name)
gs_tft <- dplyr::filter(msigdb_gene_sets, gs_cat == "C3:TFT:GTRD")
gs_tft <- split(x = gs_tft$gene_symbol, f = gs_tft$gs_name)
...
stgeomx_gsea_pipeline <- function(gsea, de, method) {
  ...
  res <- fgsea_run_batch(de, gs_gobp,
                       fgsea_eps=0, fgsea_nperms=gsea_nperms)
  res <- res %>% mutate(method = method)
  gsea <- bind_rows(gsea, res)
  
  res <- fgsea_run_batch(de, gs_3ca,
                       fgsea_eps=0, fgsea_nperms=gsea_nperms)
  res <- res %>% mutate(method = method)
  gsea <- bind_rows(gsea, res)
  
  res <- fgsea_run_batch(de, gs_cp,
                       fgsea_eps=0, fgsea_nperms=gsea_nperms)
  res <- res %>% mutate(method = method)
  gsea <- bind_rows(gsea, res)
  
  res <- fgsea_run_batch(de, gs_tft,
                       fgsea_eps=0, fgsea_nperms=gsea_nperms)
  res <- res %>% mutate(method = method)
  gsea <- bind_rows(gsea, res)
...
# write results for clusters
contrasts <- c("clusterPanc", "clusterBrain", "clusterLn", "clusterKidney", "clusterColon")
manuscript_pathways <- c(names(gs_hallmark), names(gs_gobp), names(gs_3ca), names(gs_cp), names(gs_tft))
res <- gsea_all %>% filter(contrast %in% contrasts,
                           pathway %in% manuscript_pathways)
